...
{
	$.ajax({
		url: '".Yii::app()->getUrlManager()->createUrl('gii/model/GetTableColunmConfig')."',
...
$form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row model-class" id="moreopt">
		<?php 
		if ($model->tableName != '' && !empty($columns))
			$this->renderPartial('moreoption', array('columns' => $columns, 'model' => $model));
		?>
