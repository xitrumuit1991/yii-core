...
class CCodeGenerator extends CController
{
        ...
        public function actionGetAttributes()
	{
		...
		$model_name = $_POST['model'];
                $a = str_replace(' ', '', $_POST['action']);
		$action = explode(',', $a);
		$model = new $model_name;
                $relation = $model->relations();                
		$class = @Yii::import($model_name, true);
		$table = CActiveRecord::model($class)->tableSchema;
		$this->renderPartial('/admin/get_attributes', array('model' => $model, 'action' => $action, 'table' => $table, 'relation' => $relation));
	}
