...
$('#clearsearch').click(function(){
	...
	$.fn.yiiGridView.update('state-grid', {data: data});
	return false;
});

$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"state-grid_c0[]\"]:checked').length > 0;
        ...
        {
                document.getElementById('state-grid-bulk').action='" . Yii::app()->createAbsoluteUrl('admin/' . Yii::app()->controller->id  . '/deleteall') . "';
                document.getElementById('state-grid-bulk').submit();
        }
});
...
Yii::app()->clientScript->registerScript('ajaxupdate', "
    $('#state-grid a.ajaxupdate').live('click', function() {
        $.fn.yiiGridView.update('state-grid', {
            type: 'POST',
            url: $(this).attr('href'),
            success: function() {
