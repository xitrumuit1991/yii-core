...
$('#clearsearch').click(function(){
	...
	$.fn.yiiGridView.update('job-grid', {data: data});
	return false;
});

$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"job-grid_c0[]\"]:checked').length > 0;
        ...
        {
                document.getElementById('job-grid-bulk').action='" . Yii::app()->createAbsoluteUrl('admin/' . Yii::app()->controller->id  . '/deleteall') . "';
                document.getElementById('job-grid-bulk').submit();
        }
});
...
Yii::app()->clientScript->registerScript('ajaxupdate', "
    $('#job-grid a.ajaxupdate').live('click', function() {
        $.fn.yiiGridView.update('job-grid', {
            type: 'POST',
            url: $(this).attr('href'),
            success: function() {
