...
{
    public $pluralTitle = 'Enquiry Management';
    public $singleTitle = 'Enquiry';
        ...
        try {
            ...
            if(isset($_GET['Enquiry'])){
                ...
                if($model->created_date !=''){
                    $model->created_date = date('Y-m-d',strtotime(str_replace('/','-', $model->created_date)));
                }
            }
	...
	*/
	...
	{
		$deleteItems = $_POST['enquiry-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
						Enquiry::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
